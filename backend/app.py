...
import requests  # used for GeoIP enrichment

# -------------------- Configuration --------------------
DB_FILE = os.environ.get("HONEYPOT_DB", "honeypot.db")
GEOIP_URL = os.environ.get(
    "GEOIP_URL",
    "https://ip-api.com/json/{ip}?fields=status,country,city,regionName,lat,lon,isp,org,as,query",
)
...
# Setup CORS if available, otherwise permissive headers will be added implicitly
if CORS:
    CORS(app, origins="*")
...
socketio = SocketIO(
    ...
    async_mode=ASYNC_MODE or "threading",
    logger=DEBUG_MODE,
...
def get_db_connection():
    """
    Returns a sqlite3 connection. check_same_thread=False is used because
    background tasks (SocketIO threads/eventlet greenlets) may share access.
    """
...
def init_db():
    ...
    try:
        ...
        try:
            ...
            logger.debug("Could not set WAL mode; continuing with default journal mode")
        cursor = conn.cursor()
        cursor.execute(
            """
        ...
        conn.commit()
        logger.info("Database initialized (WAL if supported) and tables ensured")
    finally:
        conn.close()
...
def extract_client_ip(req) -> str:
    ...
    Returns a validated IP string; on validation failure returns '0.0.0.0'.
    """
    ip = None
    ...
    try:
        ...
        logger.warning("Invalid client IP detected: %s, defaulting to 0.0.0.0", repr(ip))
        return "0.0.0.0"
...
def insert_attack_minimal(attack_data: dict):
...
def update_attack_enrichment(attack_id: int, geo: dict | None):
    ...
    try:
        ...
        if geo:
            cur.execute(
                """
                UPDATE attacks
                   SET country=?, city=?, region=?, latitude=?, longitude=?, isp=?, org=?, asn=?
                 WHERE id=?
            """,
                (
                    geo.get("country"),
...
def honeypot_endpoint(path):
    ...
    try:
        ...
        # Emit minimal info immediately (clients can show quick feedback)
...
if __name__ == "__main__":
    ...
    # socketio.run will select the appropriate async mode / server
